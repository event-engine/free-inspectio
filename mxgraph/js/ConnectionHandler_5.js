...
mxConnectionHandler.prototype.mouseMove = function(sender, me)
{
    if (!me.isConsumed() && (this.ignoreMouseDown || this.first != null || !this.graph.isMouseDown))
    {
        ...
        if (this.first != null)
        {
            ...
            {
                if(this.previous != null && this.currentState != null && this.previous !== this.currentState) {
...
mxConnectionHandler.prototype.connect = function (source, target, evt, dropTarget) {
    ...
    if(this.insertedEdgeSourceConstraint) {
    ...
    if(this.insertedEdgeTargetConstraint) {
    ...
    if(this.lastInsertedEdge) {
        this.graph.getModel().beginUpdate();

        try {
            var edgeState = this.graph.getView().getState(this.lastInsertedEdge);
            var sourceParent = this.graph.getModel().getParent(source);
            ...
            if(edgeState) {
                ...
                if(geo && absPoints && absPoints.length > 2) {
                    ...
                    if(sourceParent && !this.graph.getModel().isRoot(sourceParent) && sourceParent === this.graph.getModel().getParent(target)) {
                        geo.translate(sourceParent.getGeometry().x * -1, sourceParent.getGeometry().y * -1);
                    }
